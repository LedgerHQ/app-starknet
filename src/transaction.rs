...
pub fn set_tx_calldata(data: &[u8], ctx: &mut Ctx, c: usize) -> Result<bool, ()> {
    ...
    /* display calldata info start */
    printf(">>>>>>>>>>>>>>>>>>>>>\n");
    printf_fe("to: ", &ctx.tx_info.calldata.calls[c].to);
    printf(core::str::from_utf8(&ctx.tx_info.calldata.calls[c].entry_point).unwrap());
    printf("\n");
    printf_fe("selector: ", &ctx.tx_info.calldata.calls[c].selector); 
    /* display calldata info end */
